...
from generate_approximation_for_all import interpolate_points_to_line, dataset_sensitive_c, get_min_and_max
...
def create_permutations(points_x, points_y, max_y, min_y, k=10 ** 6, e=0.08):
...
def get_local_robust_score(ts_length, in_points_x, points_y, model_name, k=10 ** 6, random_seed=42,
    ...
    points_x = [0] + in_points_x[1:-1] + [ts_length - 1]
    points_x[0] = 0  # Always select the first point
    points_x[-1] = ts_length - 1  # We always select the last point

    min_y = min(points_y)
    max_y = max(points_y)

    all_permutation_points = create_permutations(points_x=points_x, points_y=points_y, min_y=min_y, max_y=max_y, k=k)
    ...
    for perturbation_y_values in all_permutation_points:
        line_version = interpolate_points_to_line(ts_lenght=ts_length, x_selcted=points_x,
                                                  y_selcted=perturbation_y_values)
    ...
    if 1 not in dict_count and 0 in dict_count:
        return 0
    return dict_count[1] / (dict_count[0] + dict_count[1])
...
def run():
    ...
    for i, selected_x in enumerate(all_selected_points):
        ...
        if robustness_score < min_robustness_score:
            ...
            min_idx = i
    min_points_line = interpolate_points_to_line(ts_lenght=len(ts), x_selcted=min_robustness_points,
                                                 y_selcted=[ts[i] for i in min_robustness_points])
    max_points_line = interpolate_points_to_line(ts_lenght=len(ts), x_selcted=max_robustness_points,
                                                 y_selcted=[ts[i] for i in max_robustness_points])
